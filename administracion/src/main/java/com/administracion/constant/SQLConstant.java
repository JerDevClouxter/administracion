...
public static final String SELECT_EMPRESAS_BASE = "SELECT e.ID_EMPRESA,e.NIT_EMPRESA,e.RAZON_SOCIAL FROM EMPRESAS e ORDER BY e.RAZON_SOCIAL";
	...
	public static final String SELECT_CONSULTAR_USUARIO_TIP_NUM_DOC = "SELECT P.ID_PERSONA, P.ID_TIPO_DOCUMENTO, P.NUMERO_DOCUMENTO, P.PRIMER_NOMBRE, P.SEGUNDO_NOMBRE, "
			...
			+ " FROM PERSONAS P JOIN USUARIOS U ON P.ID_PERSONA = U.ID_USUARIO ";
	
	/** ORDER BY para obtener los USUARIOS */
	public static final String ORDER_USUARIOS = " ORDER BY P.PRIMER_APELLIDO";
	...
	public static final String SQL_GET_DOMICILIOS_VALORES =
		...
		+ "LEFT JOIN LOCALIDADES LO ON(LO.ID_LOCALIDAD=DV.ID_LOCALIDAD)"
		+ "JOIN CIUDADES CI ON(CI.ID_CIUDAD=DV.ID_CIUDAD)"
		+ "JOIN DEPARTAMENTOS DE ON(DE.ID_DEPARTAMENTO=CI.ID_DEPARTAMENTO)";

	/** ORDER BY para obtener los domicilios valores */
	public static final String ORDER_DOMICILIOS_VALORES = " ORDER BY DE.NOMBRE,CI.NOMBRE,LO.NOMBRE";

	/** Insert para la tabla DOMICILIOS_VALORES */
	public static final String INSERT_DOMICILIOS_VALORES = "INSERT INTO DOMICILIOS_VALORES(ID_LOCALIDAD,ID_CIUDAD,ZONA,VALOR,ID_ESTADO)VALUES(?,?,?,?,?)";

	/** Update para la tabla DOMICILIOS_VALORES */
	public static final String UPDATE_DOMICILIOS_VALORES = "UPDATE DOMICILIOS_VALORES SET ID_LOCALIDAD=?,ID_CIUDAD=?,ZONA=?,VALOR=?,ID_ESTADO=? WHERE ID_VALOR=?";

	/** Count para la tabla DOMICILIOS_VALORES filtro ciudad */
	public static final String COUNT_DOMICILIOS_VALORES_CIUDAD = "SELECT COUNT(*)FROM DOMICILIOS_VALORES WHERE ID_CIUDAD=? AND UPPER(ZONA)=UPPER(?) AND ID_LOCALIDAD IS NULL";
	...
	public static final String INSERT_DELIVERY_VEHICULO = "INSERT INTO DELIVERIES_VEHICULOS(ID_DELIVERY,PLACA,ID_TIPO_VEHICULO,ID_FABRICANTE,CILINDRAJE)VALUES(?,?,?,?,?)";
	...
	/** SQL para obtener el detalle del delivery */
	public static final String GET_DETALLE_DELIVERY =
		"SELECT "
			+ "D.ID_DELIVERY AS A,"
			+ "T.ID_TIPO_DOCUMENTO AS B,"
			+ "T.DESCRIPCION AS C,"
			+ "P.NUMERO_DOCUMENTO AS D,"
			+ "P.PRIMER_NOMBRE AS E,"
			+ "P.SEGUNDO_NOMBRE AS F,"
			+ "P.PRIMER_APELLIDO AS G,"
			+ "P.SEGUNDO_APELLIDO AS H,"
			+ "P.CORREO_ELECTRONICO AS I,"
			+ "P.TELEFONO AS J,"
			+ "P.FECHA_NACIMIENTO AS K,"
			+ "CIU.ID_CIUDAD AS L,"
			+ "CIU.NOMBRE AS M,"
			+ "P.GENERO AS N,"
			+ "D.ID_ESTADO AS O,"
			+ "DE.ID_DELIVERY_EQUIPO AS P,"
			+ "DE.ID_FABRICANTE AS Q,"
			+ "FDE.NOMBRE AS R,"
			+ "DE.MODELO AS T,"
			+ "DE.NUMERO_SIM AS U,"
			+ "DE.NUMERO_IMEI AS V,"
			+ "DVE.ID_DELIVERY_VEHICULO AS X,"
			+ "TVE.ID_TIPO_VEHICULO AS Y,"
			+ "TVE.NOMBRE AS Z,"
			+ "DVE.PLACA AS S,"
			+ "FDVE.ID_FABRICANTE AS W,"
			+ "FDVE.NOMBRE AS AA,"
			+ "DVE.CILINDRAJE AS BB "
		+ "FROM DELIVERIES D "
		+ "JOIN PERSONAS P ON(P.ID_PERSONA=D.ID_DELIVERY)"
		+ "LEFT JOIN TIPOS_DOCUMENTOS T ON(T.ID_TIPO_DOCUMENTO = P.ID_TIPO_DOCUMENTO)"
		+ "LEFT JOIN CIUDADES CIU ON(CIU.ID_CIUDAD=P.CIUDAD_NACIMIENTO)"
		+ "LEFT JOIN DELIVERIES_EQUIPOS DE ON(DE.ID_DELIVERY=D.ID_DELIVERY)"
		+ "LEFT JOIN FABRICANTES FDE ON(FDE.ID_FABRICANTE=DE.ID_FABRICANTE)"
		+ "LEFT JOIN DELIVERIES_VEHICULOS DVE ON(DVE.ID_DELIVERY=D.ID_DELIVERY)"
		+ "LEFT JOIN TIPOS_VEHICULOS TVE ON(TVE.ID_TIPO_VEHICULO=DVE.ID_TIPO_VEHICULO)"
		+ "LEFT JOIN FABRICANTES FDVE ON(FDVE.ID_FABRICANTE=DVE.ID_FABRICANTE)"
		+ "WHERE D.ID_DELIVERY=?";

	/** Update para la tabla persona asociado a un delivery */
	public static final String UPDATE_DELIVERY_PERSONA = "UPDATE PERSONAS SET ID_TIPO_DOCUMENTO=?,NUMERO_DOCUMENTO=?,PRIMER_NOMBRE=?,SEGUNDO_NOMBRE=?,PRIMER_APELLIDO=?,SEGUNDO_APELLIDO=?,TELEFONO=?,CORREO_ELECTRONICO=?,FECHA_NACIMIENTO=?,CIUDAD_NACIMIENTO=?,GENERO=? WHERE ID_PERSONA=?";

	/** Update para la tabla delivery */
	public static final String UPDATE_DELIVERY = "UPDATE DELIVERIES SET ID_ESTADO=? WHERE ID_DELIVERY=?";

	/** Update para la tabla delivery DELIVERIES_EQUIPOS*/
	public static final String UPDATE_DELIVERY_EQUIPO = "UPDATE DELIVERIES_EQUIPOS SET ID_FABRICANTE=?,MODELO=?,NUMERO_SIM=?,NUMERO_IMEI=? WHERE ID_DELIVERY_EQUIPO=?";

	/** Update para la tabla delivery VEHICULO*/
	public static final String UPDATE_DELIVERY_VEHICULO = "UPDATE DELIVERIES_VEHICULOS SET PLACA=?,ID_TIPO_VEHICULO=?,ID_FABRICANTE=?,CILINDRAJE=? WHERE ID_DELIVERY_VEHICULO=?";
}
